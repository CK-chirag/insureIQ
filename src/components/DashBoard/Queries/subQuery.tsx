...
const SubQueries: React.FC = () => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm p-6 w-full" style={{ fontFamily: 'DM Sans, ui-sans-serif, system-ui, sans-serif' }}>
            ...
            <div className="grid grid-cols-4 gap-x-10 gap-y-4 pb-4 border-b border-gray-100 mb-4">
            ...
            <div className="space-y-0">
                {requests.map((request, index) => (
                    <div key={index} className={`grid grid-cols-4 gap-x-10 items-center py-4 ${index < requests.length - 1 ? 'border-b border-gray-200' : ''}`}>
                        {/* Request ID */}
                        ...
                        {/* Phone Number */}
                        <div>
                            <span className='inline-flex items-center py-1 text-sm text-gray-700 font-semibold'>
                                {request.agent.phone}
                            </span>
                        </div>

...
const PolicyCardInd: React.FC = () => {
    ...
    return (
        <div
            className="relative z-0 bg-white rounded-xl border-2 border-[var(--color-custom-blue)] p-6 shadow-md transition-all duration-200 cursor-pointer hover:scale-[1.02] hover:shadow-lg flex flex-col h-full"
            style={{ minWidth: 320, maxWidth: 400, fontFamily: 'DM Sans, ui-sans-serif, system-ui, sans-serif' }}
            onClick={() => navigate("/policy-details")}
        >
            {/* Popular badge */}
            <div className="absolute -top-4 left-0 w-[315px] flex justify-center z-10">
            ...
            <div className="mt-auto">
                <Button text="Download" type="button" className="w-full bg-[var(--color-custom-blue)] text-white font-normal py-3 rounded-lg text-base" />
            </div>
        </div>
    );
};
